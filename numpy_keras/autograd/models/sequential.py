...
class Sequential:
    ...
    def fit(
        ...
        loss_grad = grad(self.__criterion)
        ...
        for epoch in utils.conditional_tqdm(range(initial_epoch, epochs), verbose == 1):
            ...
            for batch in utils.conditional_tqdm(batches, verbose == 2):
                ...
                loss[step] = self.__criterion(params, X_batch, y_batch)
    ...
    def __criterion(
        ...
        self.__set_params(params)
        y_hat = self.__forward(X, is_training=True)
        return self.__loss_func(y, y_hat)
        ...
        for idx, layer in self.layers.items():
            if hasattr(layer, 'params'):
                for key in layer.params:
                    layer.params[key] = params[f"{idx}_{key}"]
    ...
    @property
    def output_dim(self):
        return self.layers[list(self.layers.keys())[-1]].output_dim